...
class ChromaCollectionCreator:
    ...
    def create_chroma_collection(self):
        ...
        if documents:
            ...
            st.warning("No text chunks were created. Check if the documents contain text.", icon="⚠️")
            return
        ...
        self.db = Chroma.from_documents(documents, self.embed_model, persist_directory= "embed_db")
    ...
    def as_retriever(self):
        """
        Returns a retriever object for querying the Chroma collection.
        """
        if not self.db:
          raise ValueError("Chroma Collection has not been created.")
        return self.db  # Use the Chroma object directly as a retriever if compatible
